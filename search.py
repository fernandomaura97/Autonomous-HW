...
def depthFirstSearch(problem: SearchProblem):
    ...
    stack_position.push((problem.getStartState(),[]))

    while(True):

        # No solution found
        if stack_position.isEmpty():
            return []

        xy,path = stack_position.pop()
        visited.append(xy)
        ...
        if succ:
            for item in succ:
                if item[0] not in visited:
                    ...
                    stack_position.push((item[0],newPath))
...
def breadthFirstSearch(problem: SearchProblem):
    ...
    # Start from the beginning and find a solution
    queue_position.push((problem.getStartState(),[]))

    while(True):

        # No solution found
        if queue_position.isEmpty():
            return []


        xy,path = queue_position.pop()
        visited.append(xy)
        ...
        if succ:
            for item in succ:
                if item[0] not in visited and item[0] not in (state[0] for state in queue_position.list):

                    newPath = path + [item[1]] # Calculate new path
                    queue_position.push((item[0],newPath))
...
def uniformCostSearch(problem: SearchProblem):
    ...
    # queue_position: ((x,y),[path],priority)
    queue_position = PriorityQueue()
    ...
    queue_position.push((problem.getStartState(),[]),0)
    ...
    while(True):
        ...
        if succ:
            for item in succ:
                if item[0] not in visited and (item[0] not in (state[2][0] for state in queue_position.heap)):


                    newPath = path + [item[1]]
                    pri = problem.getCostOfActions(newPath)

                    queue_position.push((item[0],newPath),pri)

                # State is in queue. Check if current path is cheaper from the previous one
                elif item[0] not in visited and (item[0] in (state[2][0] for state in queue_position.heap)):
                    for state in queue_position.heap:
                        if state[2][0] == item[0]:
                    ...
                    if oldPri > newPri:
                        newPath = path + [item[1]]
                        queue_position.update((item[0],newPath),newPri)
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    # queue_position: ((x,y),[path]) #
    queue_position = MyPriorityQueueWithFunction(problem,f)
    ...
    queue_position.push(element,heuristic)
